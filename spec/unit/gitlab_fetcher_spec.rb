...
describe GitLabChangelogGenerator::GitlabFetcher do
  let(:options) do
    {
      user: "kreczko",
      project: "changelog-testrepo",
      github_endpoint: "https://gitlab.com/api/v4"
    }
  end

  let(:fetcher) { GitLabChangelogGenerator::GitlabFetcher.new(options) }

  describe "#check_response" do
    context "when returns successfully" do
      it "returns block value" do
        expect(fetcher.send(:check_response) { 1 + 1 }).to eq(2)
      end
    end

    context "when raises GitLab::MissingCredentials" do
      it "aborts" do
        expect(fetcher).to receive(:sys_abort).with("Error: wrong AUTH token")
        fetcher.send(:check_response) { raise(GitLab::MissingCredentials) }
      end
    end

    context "when raises GitLab::Forbidden" do
      it "sleeps and retries and then aborts" do
        retry_limit = GitLabChangelogGenerator::GitlabFetcher::MAX_FORBIDDEN_RETRIES - 1
        ...
        fetcher.send(:check_response) { raise(GitLab::Forbidden) }
      end
    end
  end
  ...
  describe "#get_all_tags" do
    context "when fetch_tags returns tags" do
      it "returns tags" do
        mock_tags = ["tag"]
        allow(fetcher).to receive(:fetch_tags).and_return(mock_tags)
  ...
  describe "#fetch_tags" do
    context "when wrong token provided", :vcr do
      ...
      it "should raise Unauthorized error" do
        expect { fetcher.fetch_tags }.to raise_error SystemExit, "Error: wrong AUTH token"
      end
    end

    # TODO: swap for Gitlab API
    context "when API call is valid", :vcr do
      it "should return tags" do
        ...
        expect(fetcher.fetch_tags).to eq(expected_tags)
      end
